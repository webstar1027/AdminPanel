    ...
    <div class="col-xs-12 col-sm-12 workout-calender-chart-left">
        <div class="portlet box green">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-calendar-plus-o"></i><?php echo __("My Calendar"); ?> 
                </div>
                <div class="caption pull-right">
                    <a href="<?php echo $authUrl ?>" class="btn btn-success"><i class="fa fa-refresh"></i> <?php echo __("Sync With Google Calendar"); ?></a>
                </div>
            </div>
            ...
            <div class="modal-body">
                <div class="portlet light portlet-fit bordered">
                    ...
                    <div class="portlet-body">
                        ...
                        <div>
                            <img id="measurementPhotoId" src="<?php echo $this->request->base; ?>/webroot/upload/profile-placeholder.png" width="150" title="" class="img-responsive">
                        </div>
        ...
        <div class="modal-content modal-lg">
            ...
            <div class="modal-body" id="createPtOpt_modal_body">
                ...
                <div class="form-body" id="at_work_div">
                    ...
                    $min = ["00" => "00", "05" => "05", "10" => "10", "15" => "15", "20" => "20", "25" => "25", "30" => "30", "35" => "35", "40" => "40", "45" => "45", "50" => "50", "55" => "55"];
                    $ampm = ["AM" => "AM", "PM" => "PM"];
                    ?>
                    <div class="form-group form-md-line-input">
    ...
    $(document).ready(function () {
        $('#calendars').fullCalendar({
                    ...
                    success: function (result) {
                        if (result.status == 'success') {
                            ...
                            if (result.title == 'Measurement') {
                                ...
                                if(photoId == null){
                                    photoId = "<?php echo $this->request->base; ?>upload/profile-placeholder.png";
                                }else{
                                   photoId = "http://node.gotribefit.com/upload-measurement-Historyupload/" + photoId;
                                }
                                $('measurementPhotoId').attr('src', photoId);
                                ...
                                $('#workout_client_name').text('<?php echo strtoupper($this->Gym->get_user_name($session['original_id']));?>');
                                $('#avragePercentage').text(Math.round(((result.data.averageHr) * 100) / result.data.averageMaxHr));
            ...
            select: function (start, end, jsEvent, view) {
                ...
                var currMonth = moment(d).format('M');//numeric month
                var selectedMonth = moment(start._d).format('M');
                if(view.calendar.view.type != 'month'){
                    if ( start._d.getTime() < d.getTime() ) {
                        alert("You can not create schedule for past date.");
                        return false;
                    }
                }else if(view.calendar.view.type == 'month'){
                    if ( ( (start._d.getDate()) < (d.getDate()) ) && !(selectedMonth > currMonth) ) {
                        alert("You can not create schedule for past date.");
                        return false;
                    }
                }
                ...
                if(view.calendar.view.type == 'month'){
                    var start_time_new = moment(d).format('mm');
                    var end_time_new = moment(d).format('mm');
                    var starthr = moment(d).format('hh');
                    var endhr = moment(d).format('hh');
                }else{
                    var start_time_new = moment(start._d).format('mm');
                    var end_time_new = moment(end._d).format('mm');
                    var starthr = moment(start._d).format('hh');
                    var endhr = moment(end._d).format('hh');
                }
                //console.log(start._d);
                //var starthr = moment(start._d).format('hh');
                if( start_time_new <= 00){
                    var startmin = '00';
                }else if(start_time_new <= 05){
                    var startmin = '05';
                }else if(start_time_new <= 10){
                    var startmin = '10';
                }else if(start_time_new <= 15){
                    var startmin = '15';
                }else if(start_time_new <= 20){
                    var startmin = '20';
                }else if(start_time_new <= 25){
                    var startmin = '25';
                }else if(start_time_new <= 30){
                    var startmin = '30';
                }else if(start_time_new <= 35){
                    var startmin = '35';
                }else if(start_time_new <= 40){
                    var startmin = '40';
                }else if(start_time_new <= 45){
                    var startmin = '45';
                }else if(start_time_new <= 50){
                    var startmin = '50';
                }else if(start_time_new <= 55){
                    var startmin = '55';
                }else if(start_time_new <= 59){
                    var startmin = '00';
                    starthr = parseInt(starthr) +1;
                }else{
                    var startmin = start_time_new;
                }
                
                //var endhr = moment(end._d).format('hh');
                if(moment(end._d).format('mm') <= 00){
                    var endmin = '00';
                }else if(end_time_new <= 05){
                    var endmin = '05';
                }else if(end_time_new <= 10){
                    var endmin = '10';
                }else if(end_time_new <= 15){
                    var endmin = '15';
                }else if(end_time_new <= 20){
                    var endmin = '20';
                }else if(end_time_new <= 25){
                    var endmin = '25';
                }else if(end_time_new <= 30){
                    var endmin = '30';
                }else if(end_time_new <= 35){
                    var endmin = '35';
                }else if(end_time_new <= 40){
                    var endmin = '40';
                }else if(end_time_new <= 45){
                    var endmin = '45';
                }else if(end_time_new <= 50){
                    var endmin = '50';
                }else if(end_time_new <= 55){
                    var endmin = '55';
                }else if(end_time_new <= 59){
                    var endmin = '00';
                    endhr = parseInt(endhr) +1;
                }else{
                    var endmin = end_time_new;
                }
                
                $('#start_hrs').val(starthr).prop('selected', true);
                $('#start_min').val(startmin).prop('selected', true);
                $('#start_ampm').val(moment(start._d).format('A')).prop('selected', true);
                
                $('#start_hrs_avail').val(starthr).prop('selected', true);
                $('#start_min_avail').val(startmin).prop('selected', true);
                ...
                $('#end_hrs_avail').val(endhr);
                $('#end_min_avail').val(endmin);
...
<script>
    function initializeChart(zonesDuration, duration) {
        ...
        AmCharts.makeChart("workout_chart", {
            ...
            "valueAxes": [{
                    "position": "left",
                    "axisAlpha": (zonesDurationArr[0])/(60).toFixed(2),
                    "gridAlpha": 0,

                 ...
                 <div class="portlet-title">
                     ...
                     <div class="top">
                         ...
                         <div class="btn-set pull-right">
                             <a href="<?php echo $this->Gym->createurl("PheramorUser","viewMember/".$data['user_id']); ?>" class="btn blue"><i class="fa fa-bars"></i> <?php echo __("Subscription Payment List"); ?></a>
		...
		<!--<input type="hidden" name="created_by" value="1">-->
				...
				echo @$this->Form->select("user_id",$members,["default"=>($edit)?$data['user_id']:'',"empty"=>__("Select Member"),"class"=>"form-control validate[required]",($edit)?"disabled":""]);
                    ...
                    <div class="form-group form-md-line-input">
                        ...
                        <div class="col-md-9">
                            <?php
                            echo $this->Form->select("subscription_id", $membership, ["default" => ($edit) ? $data["subscription_id"] : "", "empty" => __("Select Membership"), "class" => "form-control input-text gen_membership_id validate[required]", "disabled", "data-url" => $this->request->base . "/PheramorAjax/get_amount_by_memberships"]);
                            ?>
                            <div class="form-control-focus"> </div>
                    ...
                    <div class="form-group form-md-line-input">
                        ...
                        <div class="col-md-9">
                            <?php if($data["plan_status"]=='4')  
                                  {
                                  echo "<input type='hidden' name='mem_plan_status' value='4'>";
                                  echo "<span class='label label-info'>Unsubscribe</span>";
                                  }else { ?>
                                <select name="plan_status" class="form-control input-text validate[required]">
                               <option value="">Membership Status</option>
                               <option value="0" <?php if($data["plan_status"]=='0'){ echo "selected";}?>>Disable</option>
                               <option value="1" <?php if($data["plan_status"]=='1'){ echo "selected";}?>>Active</option>
                               <option value="2" <?php if($data["plan_status"]=='2'){ echo "selected";}?>>Upgraded</option>
                               <option value="3" <?php if($data["plan_status"]=='3'){ echo "selected";}?>>Expired</option>
                             </select> 
                                  <?php } ?>
                            <div class="form-control-focus"> </div>

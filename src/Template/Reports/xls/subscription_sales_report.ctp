...
$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, 'Subscription Name');
$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, 'Subscription Price');
...
$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, 'Plan Status');
...
if (!empty($users)) {
     foreach ($users as $row) {
         ...
         if ($row['plan_status'] == 1) {$planstaus='Active';}else if($row['plan_status'] == 3){$planstaus='Expired';}else if($row['plan_status'] == 4){$planstaus='Unsubscribe';}else {$planstaus='--';}
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('D'.$i, "$ ".$row['subscription_amount']);
          ...
          $objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $planstaus);

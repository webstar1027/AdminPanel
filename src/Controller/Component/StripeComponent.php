...
class StripeComponent extends Component
{
   ...
   // public $mode = 'Test';
     ...
     * @var string
     */
   // public $key = 'sk_test_Bid87wt43Qu5pSKMMDaQJ5sU';
    public $key = '';
    ...
    public function initialize(array $config)
    {
        ...
        $setting_table = TableRegistry::get("PheramorGeneralSetting");
        $setting_data = $setting_table->find('all')->where(['id' => 1])->first();
         if($setting_data['enable_sandbox']==0){
             $this->mode ='Test';
             $this->key=$setting_data['test_secret_key'];
         }else{
             $this->mode ='Live';
             $this->key=$setting_data['live_secret_key'];
         }
     ...
     * @link https://stripe.com/docs/api/php#retrieve_customer
     */
    public function defaultSetcard($customerId = null,$cardId = null) {
        if (!$customerId) {
            throw new Exception(__('Customer Id is not provided'));
        }
      if (!$cardId) {
